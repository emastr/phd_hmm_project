...
dists = np.zeros_like(n_pts_grid).astype(float)
u_tru = F(z)
...
for idx, n_pts in enumerate(n_pts_grid):
    ...
    arger = np.argwhere(rel_err > 0.2).flatten()[0]
    dists[idx] = np.abs(z[arger] - z[-1])
    print(idx, np.abs(z[arger] - z[-1]), arger)
    #plt.plot(rel_err)

fig = plt.figure()
plt.plot(1/n_pts_grid, 2.8/n_pts_grid, 'red', label="viable distance =2.8 discretisation distance")
plt.scatter(1/n_pts_grid, dists, color='black', label="data")
plt.xscale('log')
plt.yscale('log')
plt.ylabel("Minimal viable distance from boundary")
plt.xlabel("Distance between discretisation points")
plt.legend(edgecolor='none')
ax = plt.gca()
ax.spines["right"].set_visible(False)
ax.spines["top"].set_visible(False)
fig.savefig("boundary_integrals/figures/viable_distance_from_bdry.pdf", bbox_inches='tight')